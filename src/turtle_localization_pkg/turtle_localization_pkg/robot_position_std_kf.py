...
class LocalizationNode(Node):
    def __init__(self):
        ...
        # Publishers
        self.pub1 = self.create_publisher(Pose, '/Estimated_Position_std_kf', 10)
        ...
        x = np.array([5.5, 5.5])  # Initial state [x, dx]
        P = np.diag([1, 1])*1e-3  # Initial uncertainty
        R = self.noise_variance  # Measurement noise covariance
        Q = self.noise_variance   # Process noise variance
